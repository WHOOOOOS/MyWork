...
//来自付峙嘉的注释：这个函数HAL_SRAM_MspInit由于移植，进行必要的注释处理
...
void LCD_Init(void)
{
//来自：付峙嘉的注释：以下是移植过程中造成的一些必要的注释，主要原因是因为这些内容和其他定义部分有些许重复
//    GPIO_InitTypeDef GPIO_Initure;
...
//    LCD_LED = 1;                //点亮背光
HAL_GPIO_WritePin(GPIOB,GPIO_PIN_0,GPIO_PIN_SET);//更改后的点亮背光，移植处理的结果
    LCD_Clear(WHITE);
}  
//清屏函数
//color:要清屏的填充色
...
函数功能：画点函数
****************************************************/
void LCD_DrawPoint_user(uint16_t x,uint16_t y,uint16_t c)
{	 
	LCD_SetCursor(x,y);		//设置光标位置 
	LCD_WriteRAM_Prepare();	//开始写入GRAM
	LCD->LCD_RAM=c; 
